...
/**
 * Connector connects to an Accumulo instance and allows the user to request
 * readers and writers for the instance as well as various objects that permit
 * administrative operations.
 * 
 * The Connector enforces security on the client side by forcing all API calls
 * to be accompanied by user credentials.
 */
public abstract class Connector {
   ...
   *          column visibility of each key in order to filter data. The
   *          authorizations passed in must be a subset of the accumulo user's
   *          set of authorizations. If the accumulo user has authorizations
   *          (A1, A2) and authorizations (A2, A3) are passed, then an exception
   *          will be thrown.
   ...
   *           when the specified table doesn't exist
   * @deprecated since 1.5.0; Use
   *             {@link #createBatchWriter(String, BatchWriterConfig)} instead.
   */
  @Deprecated
   ...
   * Multi-table batch writers can queue data for multiple tables, which is good
   * for ingesting data into multiple tables from the same source
   * 
   * @param maxMemory
   *          size in bytes of the maximum memory to batch before writing
   * @param maxLatency
   ...
   * @deprecated since 1.5.0; Use
   *             {@link #createMultiTableBatchWriter(BatchWriterConfig)}
   *             instead.
   */
  @Deprecated
   ...
   * multiple tables can be sent to a server in a single batch. Its an efficient
   * way to ingest data into multiple tables from a single process.
   * 
   * @param config
   ...
   *           when the specified table doesn't exist
   */
  public abstract Scanner createScanner(String tableName,
      Authorizations authorizations) throws TableNotFoundException;

  public abstract <T, R, F, Q, VI, TS, V> GenericScanner<T, R, F, Q, VI, TS, V> createGenericScanner(
  ...
  /**
   * Retrieves a SecurityOperations object to perform user security operations,
