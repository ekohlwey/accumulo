...
public class GenericScannerOptions<T,F,Q> implements GenericScannerBase<T,F,Q> {
  ...
  private final EntryConverter<T, F, Q> converter;
  
  protected GenericScannerOptions(EntryConverter<T, F, Q> converter) {
    this.converter = converter;
  }
  
  public GenericScannerOptions(GenericScannerOptions<T,F,Q> so) {
    setOptions(this, so);
    this.converter = so.getConverter();
  }
  
  private EntryConverter<T, F, Q> getConverter() {
    return converter;
  }
  ...
  protected static void setOptions(GenericScannerOptions dst, GenericScannerOptions src) {
    synchronized (dst) {
      synchronized (src) {
